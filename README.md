...
**Settings → Setup → Launch at login** opens DuoFX in the menu bar after you sign in. Effects still start paused. The toggle reads macOS's actual registration status, reports failures, and links to Login Items when approval is required. It uses Apple's [main-app login service](https://developer.apple.com/documentation/servicemanagement/smappservice/mainapp); registering occurs only when you turn the toggle on.

**Settings → Effect → Custom presets** saves your current appearance and sound settings under a unique name. Select a saved preset and click **Apply**, or use **Manage** to replace it with current settings or delete it. Presets persist locally across launches. Applying one preserves working/minimum lid angles, desktop/input selection, Reduce Motion preference, and whether the effect is enabled.

**Settings → Setup → Reduce Motion** defaults to **Follow macOS**, with **Always reduce motion** and **Full animation** overrides. Reduced motion replaces the selected animation with a uniform blur/dimming fade at fixed desktop coordinates: no tilt, moving boundary, or folding shadow. The selected animation remains saved. DuoFX observes [macOS accessibility display changes](https://developer.apple.com/documentation/appkit/nsworkspace/accessibilitydisplayshouldreducemotion), including while Settings is closed. The bundled preview also shows this fade; automatic demos and preview easing are disabled whenever macOS or DuoFX requests reduced motion. Sound remains independently configurable.
